...
public class TC2122_Verify_Events_displaying_in_MyEvents_when_guest_user_registered extends BaseTest {
	@Test(dataProviderClass = DataProviders.class, dataProvider = "masterDP")
	public void TC2122(Hashtable<String, String> data) throws Exception {

	ExcelReader excel = new ExcelReader(Constants.SUITE1_XL_PATH);
	DataUtil.checkExecution("master", "TC2122", data.get("Runmode"), excel);
	log.info("Inside Login Test");
	openBrowser(data.get("browser"));
	logInfo("Launched Browser : " + data.get("browser"));
	logInfo("BizLigo Application Opened");
	HomePage home = new HomePage().open(data.get("tenantType"));
	UpcomingEventsPage UEP=home.navigateToUpComingEvents();
	UEP.registerToAnEventAsGuest(data.get("UpcomingEvent"),data.get("email"));
